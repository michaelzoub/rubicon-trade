...
The migration is supplied in the repository; local unit tests do not apply it to a hosted database. Run `npm test`, `npm run typecheck`, and `npm run build`. `/preview?view=agents` offers an in-memory UI test surface for the agent cards, the reveal dialog, waking and resting, and creation; reloading the preview resets its agents.
