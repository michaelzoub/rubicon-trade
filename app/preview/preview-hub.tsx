...
const VIEWS: Record<string, string> = { "/": "home", "/explore": "explore", "/trade": "trade", "/activity": "activity", "/agents": "agents", "/profile": "profile" };

export function PreviewHub({ view }: { view: string }) {
  const api = useMemo(() => {
    ...
    const states = new Map<string, HubState>([
      ["default", { ...structuredClone(PREVIEW_STATE), agent: { ...defaultAgent(), name: "Michael’s agent", description: "AI inference and the power that feeds it.", enabled: true } }],
    ...
    return { ...baseApi,
      agents: async () => ({ agents: [...states.values()].map(s => s.agent!), account: account() }),
      ...
      createAgent: async (_token: unknown, config: Parameters<typeof import("../(hub)/_hub/client").hubApi.createAgent>[1]) => {
        const agent = { ...config, id: crypto.randomUUID() };
        ...
        const state: HubState = { ...structuredClone(PREVIEW_STATE), agent, revision: 0, profile: { ...structuredClone(PREVIEW_STATE.profile), thesis: config.thesis, interests: [], themes: [], permission: "notify" }, chats: emptyChats(), events: [], trades: [], inferred: [], signals: [], preferences: [], dislikes: [], brokerage: { provider: "robinhood", connected: false } };
      ...
      setAgentEnabled: async (_token: unknown, id: string, enabled: boolean) => {
        ...
        return { agents: [...states.values()].map(s => s.agent!) };
      },
      deleteAgent: async (_token: unknown, id: string) => { if (states.size > 1) states.delete(id); return { agents: [...states.values()].map(s => s.agent!) }; },
      runAgent: async (_token: unknown, id: string) => {
        ...
        previewRuns.unshift(run);
        return { outcome: { runId: run.id, status: "succeeded" as const, summary: run.summary, decision: run.decision, notification: { title: "Vertiv is up 3% after raising guidance", body: "", relevance: "high" as const } }, state, runs: [...previewRuns] };
      },
      runs: async () => ({ runs: [...previewRuns] }),
  ...
  const content = view === "agents" ? <AgentsView /> : view === "explore" ? <ExploreView /> : view === "trade" ? <TradeView /> : view === "activity" ? <ActivityView /> : view === "profile" ? <ProfileView /> : view === "asset" ? <AssetDetail kind="stock" id="VRT" /> : <HomeView />;
  return <HubProvider userId={PREVIEW_USER} name="Michael" initialAccount={PREVIEW_ACCOUNT} initial={fresh ? { ...PREVIEW_STATE, chats: emptyChats(), inferred: [], events: PREVIEW_STATE.events.slice(0, 1), trades: [] } : { ...PREVIEW_STATE, agent: { ...defaultAgent(), name: "Michael’s agent", description: "AI inference and the power that feeds it.", enabled: true } }} api={api}><Hub path={ROUTES[view] ?? "/"} resolveHref={href => `/preview?view=${VIEWS[href] ?? "home"}`}>{content}</Hub></HubProvider>;
}
