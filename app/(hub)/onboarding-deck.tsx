...
import { CATEGORIES, predictions, splitPrediction } from "@/lib/socialtrading/onboarding";
...
export type DeckCard = { id: string; category: string; text: string; lead?: string; horizon?: string };
...
function stackStyle(rise: number): CSSProperties {
  return { transform: `translateY(${rise * 12}px) scale(${1 - rise * .04})`, opacity: 1 - rise * .22 };
}

/** A deck you throw rather than a form you fill. The top card stays straight
 * until it moves: left and right tilt, down shrinks, and the matching label
 * is waiting behind it. */
export function PredictionDeck({ card, backs = 1, answered, total, onVote, onUndo }: {
  ...
  const split = splitPrediction(card.text);
  const horizon = split.horizon ?? card.horizon;
  const claim = split.horizon ? split.claim : card.text;
  const under = Math.max(1, Math.min(2, backs));
  ...
  async function fly(direction: Direction) {
    ...
    if (el && !quiet) {
      ...
      await gsap.to(el, {
        x, y,
        rotation: direction === "yes" ? 26 : direction === "no" ? -26 : 0,
        opacity: 0,
        scale: direction === "unsure" ? 0.82 : 0.92,
        duration: rubiconMotion.duration.state,
        ease: "power3.in",
      });
    }
  ...
  useGSAP(() => {
    ...
    paint(stage, 0, 0);
    if (quiet) return;
    gsap.set(el, { x: 0, y: 0, rotation: 0, opacity: 1, scale: 1, transformOrigin: "50% 50%" });
    gsap.fromTo(el, { y: 22, rotation: 0, scale: 0.97, opacity: 0 }, {
      y: 0, rotation: 0, scale: 1, opacity: 1, duration: 0.42, ease: "creature",
    });
    invite.current = gsap.to(el, {
      x: 16, duration: 0.72, ease: "sine.inOut",
      yoyo: true, repeat: 7, delay: 0.45, repeatDelay: 0.18,
    });
    ...
    const [drag] = Draggable.create(el, {
      ...
      onPress() {
        stopHint();
        if (!locked.current) gsap.to(el, { scale: 1.02, duration: rubiconMotion.duration.micro, ease: "power2.out" });
      },
      onDrag() {
        if (locked.current) return;
        const pose = poseOf(this.x, this.y);
        gsap.set(el, { rotation: pose.rotation, scale: pose.scale });
      ...
      onRelease() {
        ...
        gsap.to(el, { x: 0, y: 0, rotation: 0, scale: 1, duration: 0.62, ease: "elastic.out(1, 0.62)", onUpdate: () => paint(stage, Number(gsap.getProperty(el, "x")), Number(gsap.getProperty(el, "y"))) });
      },
    });
    return () => { invite.current?.kill(); drag.kill(); };
  }, { dependencies: [card.id], revertOnUpdate: true });

  return <div ref={deck} className="onb-deck" data-sector={sector} style={{ "--force": 0 } as CSSProperties} aria-busy={exiting}>
    <div className="onb-deck-stage">
      {Array.from({ length: under }, (_, i) => {
        ...
        return <div key={`back-${i}`} className="onb-card-swipe is-back" data-sector={sector} aria-hidden="true" data-depth={i + 1}
          style={stackStyle(rise)} />;
      })}
      <div ref={top} className="onb-card-swipe" data-sector={sector} data-depth="0" tabIndex={0} role="group"
        aria-label={`${card.text} Swipe right if likely, left if unlikely, or down if unsure.`}
        ...
        {horizon ? <p className="onb-swipe-horizon">{horizon}</p> : null}
        <h2>{claim}</h2>
        <Icon className="onb-swipe-mark" strokeWidth={1} aria-hidden="true" />
      </div>
    </div>
    <button type="button" className="onb-vote" data-dir="no" disabled={exiting} onClick={() => void fly("no")}>← Unlikely</button>
    <button type="button" className="onb-vote" data-dir="yes" disabled={exiting} onClick={() => void fly("yes")}>Likely →</button>
    <button type="button" className="onb-vote" data-dir="unsure" disabled={exiting} onClick={() => void fly("unsure")}>↓ Not sure</button>
    {onUndo && <div className="onb-deck-foot">
      <button type="button" className="onb-text-button" onClick={onUndo}><RotateCcw size={13} aria-hidden="true" /> Undo last swipe</button>
    </div>}
  </div>;
}

export function DealingDeck({ backs = 1 }: { backs?: number; answered?: number; total?: number }) {
  const under = Math.max(1, Math.min(2, backs));
...
export function InferredSwipeDeck({ upcoming, knowledge, confidence, fetchDeck = fetchPredictionDeck, ...deck }: {
  ...
  if (!pack) return <DealingDeck backs={deck.backs} />;
