...
const sendTransaction = vi.fn(async ({ method }: { method: string }) =>
  method === "eth_chainId" ? "0x1" : method === "eth_accounts" ? [PREVIEW_WALLET] : `0x${"ab".repeat(32)}`);
...
const stranded = { chainId: 1, chainName: "Ethereum", wallet: PREVIEW_WALLET, token: "0xa0b86991c6218b36c1d19d4a2e9eb0ce3606eb48", symbol: "USDC", decimals: 6, balance: "4250000", display: "4.25", kind: "usdc" as const };
const posted: Record<string, unknown>[] = [];
const api = {
  crypto: async (_t: unknown, _r: number, body: Record<string, unknown>) => {
    posted.push(body);
    if (body.action === "holdings") return { holdings: [stranded] };
    if (body.action === "withdraw") return { transaction: { chainId: 1, from: PREVIEW_WALLET, to: stranded.token, data: "0xa9059cbb", value: "0", nonce: "0x3" } };
...
it("shows what is stranded and on which network, without being asked", async () => {
  await render();
  expect(container.textContent).toContain("4.25 USDC");
  expect(container.textContent).toContain("on Ethereum");
...
it("names the amount, network and destination before anything is signed", async () => {
  ...
  expect(confirm.textContent).toContain("Send 4.25 USDC on Ethereum");
...
it("sends only after the explicit confirmation, on the right network", async () => {
  ...
  expect(posted.find(p => p.action === "withdraw")).toMatchObject({ chainId: 1, wallet: PREVIEW_WALLET, to: `0x${"22".repeat(20)}`, amount: "4250000" });
