...
function normalize(c: Coin): Asset {
  return { id: c.id, symbol: c.symbol.toUpperCase(), name: c.name, kind: "crypto", source: "CoinGecko", logo: c.image, price: c.current_price, change: c.price_change_percentage_24h, marketCap: c.market_cap, volume: c.total_volume, asOf: c.last_updated, themes: ["crypto"], chart: [], news: [] };
}
...
export const cryptoData = {
  async search(query: string): Promise<Asset[]> {
    ...
    return (await gecko<Coin[]>(`/coins/markets?vs_currency=usd&per_page=15&order=market_cap_desc${ids ? `&ids=${encodeURIComponent(ids)}` : ""}`)).map(normalize);
  },
  async trending(): Promise<Asset[]> {
    ...
    return ids ? (await gecko<Coin[]>(`/coins/markets?vs_currency=usd&ids=${encodeURIComponent(ids)}`)).map(normalize) : [];
  },
  async detail(id: string, days = 30): Promise<Asset> {
    ...
    const [coins, chart, metadata] = await Promise.allSettled([
      gecko<Coin[]>(`/coins/markets?vs_currency=usd&ids=${id}`),
