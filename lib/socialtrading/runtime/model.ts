...
export function openRouterModel(planId: PlanId = "free"): ModelClient {
  return {
    async complete({ messages, tools, toolChoice, signal, maxTokens = 900 }) {
      ...
      const response = await fetch(OPENROUTER_URL, {
        ...
        body: JSON.stringify({ model, messages, ...(tools.length ? { tools, tool_choice: toolChoice } : {}), temperature: .3, max_tokens: maxTokens, ...USAGE_ACCOUNTING }),
      });
